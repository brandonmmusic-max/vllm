...
def _resolve_r7_a1_min_rows(ext: Any) -> int:
    ...
    if value < 0:
        raise ValueError(
            "VLLM_EXL3_R7_A1_MIN_ROWS must be >= 0, got " f"{value}"
        )
    return value
...
def _r7_w13_ballast(slots: int, like: torch.Tensor) -> torch.Tensor:
    ...
    key = (plane, like.dtype, like.device)
    slots = int(slots)
    need = 2 * slots
...
class Exl3Config(QuantizationConfig):
    ...
    def from_config(cls, config: dict[str, Any]) -> Exl3Config:
        ...
        if isinstance(_r7, dict):
            instance.r7_routed_experts = _r7
        # --- end glm52-mdispatch ---
        return instance
...
class Exl3MoEMethod(FusedMoEMethodBase):
        ...
        if _dbg and _dbg in str(getattr(layer, "layer_name", "")):
            ...
            for _g, _a, _k in (("w13", "suh", (0, "w1")), ("w13", "suh", (5, "w3")),
                ...
                if _tt is None:
                    ...
                    _v = _tt.to(_t.float64) if _tt.dtype != _t.int16 else _tt.to(_t.float64)
    ...
    def _prepare_r7_sparkinfer_weights(self, layer: RoutedExperts) -> bool:
        ...
        if budget is not None:
            if Exl3MoEMethod._r7_fused_layers >= budget:
                return False
            Exl3MoEMethod._r7_fused_layers += 1
        ...
        for tier_id, bits in enumerate(tier_bits):
            ...
            expected_last = 16 * bits
            if int(w13.shape[-1]) != expected_last or int(w2.shape[-1]) != expected_last:
                raise ValueError(
