...
from trellismx_b12x._lib.compiler import (
    KernelCompileSpec,
    compile as b12x_compile,
)
from trellismx_b12x._lib.intrinsics import (
...
from trellismx_b12x._lib.quant.sqg_e4m3 import sqg_xor_cheb_t12_lut
from trellismx_b12x.moe._shared.kernels.trellis_ring import (
    trellis256_lane_geom_bits as _trellis_ring_lane_geom_bits,
)
from trellismx_b12x.moe._shared.trellis_codebooks import (
    MCG,
    SQG_E4M3,
    SQG_FP16,
    validate_codebook_bits,
)
from trellismx_b12x._lib.quant.sqg_fp16_d3l import (
    sqg_fp16_d3l_descriptors,
)
from trellismx_b12x._lib.utils import current_cuda_stream, make_ptr
from trellismx_b12x.moe._shared.kernels.w4a16.route_pack import (
    pack_topk_routes_by_expert as _pack_topk_routes_by_expert,
)
from trellismx_b12x.moe._shared.kernels.w4a16.host import (
...
from trellismx_b12x._lib.runtime_control import (
    raise_if_kernel_resolution_frozen,
)
from trellismx_b12x.moe._shared.kernels.activations import (
    SITU,
    SITU_DEFAULT_BETA,
...
from trellismx_b12x.moe._shared.kernels.micro import (
...
_W4A16_REGS_SM121 = {
    # GLM-5.3 route-128/N64 prefill qualification. The two-stage kernel is
    # deliberately one-CTA/SM; use the conservative ceiling until its cubin
    # register count is sealed from ptxas.
    (256, 8, 16, 4, False): 255,
    (128, 8, 8, 4, False): 255,
    (128, 8, 4, 8, False): 255,
    (256, 1, 8, 8, True): 118,
    (256, 1, 16, 4, True): 118,
...
def _shared_memory_footprint(
    *,
    ...
    sh_a_size = int(pipeline_stages) * (cta_m * cta_k) * 2
    staged_weight_bits = int(weight_bits)
    sh_b_size = int(pipeline_stages) * (
        cta_k * cta_n * staged_weight_bits // 8
    )
...
def select_w4a16_full_rotation_launch_tile_config(
    *,
    storage_tile_config: tuple[int, int, int, int] | None,
    capacity_tokens: int,
    hidden_size: int,
    ...
    """Select a GLM decode/prefill launch over one prepared weight layout.

    Uniform-K4 Trellis storage is independent of the logical GEMM launch tile.
    Keep the prepared-weight ABI fixed, but allow the exact GLM-5.3 EP2
    prefill geometry to use its qualified two-stage SMEM/register schedule.
    Small-M decode and every nonmatching model shape retain the storage tile.
    """

    if storage_tile_config is None:
        return None
    storage_tile_config = tuple(int(v) for v in storage_tile_config)
    glm53_ep2_route128_smem = (
        os.environ.get("B12X_GL53_ROUTE128_WIDE", "0") == "1"
        and int(capacity_tokens) >= 128
        and int(hidden_size) == 4096
        and int(intermediate_size) == 2048
        and int(num_experts) == 144
        and int(topk) == 8
        and int(route_num_experts) == 288
        and int(trellis_bits) == 4
    )
    if glm53_ep2_route128_smem:
        return (64, 256, 64, 256)
    return storage_tile_config
...
class W4A16GemmKernel:
    def __init__(
        ...
        *,
        ...
        packed_route_block_factor: int = 1,
        pipeline_stages: int = _STAGES,
    ):
        ...
        self.packed_route_block_factor = int(packed_route_block_factor)
        self.pipeline_stages = int(pipeline_stages)
        if self.pipeline_stages not in (2, 3, 4):
            raise ValueError("pipeline_stages must be 2, 3, or 4")
        if self.packed_route_block_factor < 1:
            raise ValueError("packed_route_block_factor must be >= 1")
        if self.packed_route_block_factor != 1 and (
            self.direct_topk_routes or self.dense_route_fast_path
        ):
            raise ValueError(
                "packed route subtiles require route-packed expert execution"
            )
        ...
        # Experimental GLM-5.3 uniform-K4 route-128 schedule.  The ordinary
        # large-M kernel gives every warp all M16 accumulator banks and splits
        # the K reduction between warps.  At M128 that makes 256 FP32 values
        # live per lane and ptxas spills heavily.  This schedule transposes the
        # ownership: each of the eight warps owns one M16 slice for the whole K
        # reduction. Both operands remain SMEM backed and the schedule uses
        # no tensor-memory operand. It is deliberately limited to exact full-K4
        # one-CTA geometry until its numeric and timing gates pass.
        self.route128_warp_m = bool(
            self.moe_block_size == 128
            and (self.tile_n, self.tile_k) in ((64, 128), (128, 64), (256, 64))
            and self.weight_layout_trellis256
            and not self.weight_layout_trellis256_pair
            and self.trellis_bits == 4
            and self.schedule_whole_tiles
            and self.pipeline_stages == 2
        )
        self.route128_expand_b = bool(
            self.route128_warp_m and self.tile_n <= 128
        )
        # EP2 prefill usually leaves a physical M128 route block with no more
        # than 64 resident rows.  In that case the N256 schedule needs only
        # one four-warp M group.  Keep an opt-out for controlled A/B timing;
        # the value is captured in the kernel cache key below.
        self.route128_hybrid_tail = bool(
            self.route128_warp_m
            and self.tile_n == 256
            and os.getenv("B12X_GL53_ROUTE128_HYBRID_TAIL", "1") == "1"
        )
        ...
        if self.route128_warp_m:
            # The warp-owned epilogue has no cross-warp reduction and can use
            # a contiguous N64 row (8 int4 vectors) instead of the generic
            # bank-conflict padding (9).  This reclaims 2 KiB for expanded B.
            sh_red_size = 2 * self.cta_n_blocks * 16 * self.cta_m_blocks
        ...
        sh_b_size = _covering_count(
            self.pipeline_stages * self.b_sh_stage_bytes, 16
        )
        ...
        self.shared_int4 = (
            self.sh_a_off + self.pipeline_stages * self.a_sh_stage
        )
        self.sh_decode_b_off = self.shared_int4
        if self.route128_expand_b:
            # One 16 KiB BF16 exchange tile (N64/K128 or N128/K64).  The
            # ordinary reduction/output
            # region aliases compressed B and is not safe until the entire K
            # mainloop has retired, so the cooperative route-128 decoder owns
            # a distinct 16 KiB tail region.
            self.shared_int4 += (self.tile_n * self.tile_k * 2) // 16
    ...
    def __cache_key__(self) -> tuple[object, ...]:
        return (
            ...
            self.packed_route_block_factor,
            self.pipeline_stages,
            self.route128_hybrid_tail,
    ...
    def _run_persistent_gemm(
        ...
        if cutlass.const_expr(self.dense_route_fast_path):
            ...
            if cutlass.const_expr(self.packed_route_block_factor == 1):
                route_blocks = route_blocks // Int32(
                    self.schedule_route_block_factor
                )
            ...
            if (
                ...
                if cutlass.const_expr(emit_tile is not None):
                    ...
                    if cutlass.const_expr(self.dense_route_fast_path):
                        ...
                        expert_idx = block_expert_ids[
                            route_block_idx // Int32(self.packed_route_block_factor)
                        ].to(Int32)
    ...
    def _run_tile_with_pair_override(
        ...
        elif cutlass.const_expr(self.route128_warp_m):
            self._run_tile_route128_warp_m(
                a_bf16_flat,
                a_alt_bf16_flat,
                b_i32_flat,
                c_bf16_flat,
    ...
    def _run_tile_route128_warp_m(
        ...
        """Compute a spill-free M128 tile with warp-owned M/N slices.

        The N256 schedule normally uses a 2x4 M/N warp grid. Route tails with
        at most 64 live rows collapse that to one four-warp M group, avoiding
        both upper-half MMA work and its duplicate Trellis decode while the
        CTA keeps the same route-128 packing and SMEM ABI.
        """
        (
            global_scale_f32,
            block_valid_rows,
        ...
        # Whole-tile scheduling is part of the compile-time gate above; this
        # path therefore owns the complete K reduction and has no split-K
        # combine phase.

        lane = tid & Int32(31)
        warp = tid >> Int32(5)
        n_warps = self.tile_n // 64
        m_warps = (self.cta_threads // 32) // n_warps
        warp_n = warp % Int32(n_warps)
        warp_m = warp // Int32(n_warps)
        a_sh_rd = Int32(self.a_sh_stride) * (lane % Int32(16)) + lane // Int32(16)

        accs = []
        for _ in cutlass.range_constexpr(self.cta_m_blocks // m_warps):
            acc = [
                cute.make_rmem_tensor(
                    (_SCALAR_ACC_FRAGMENT_WIDTH,), cutlass.Float32
                )
        ...
        a_regs = cute.make_rmem_tensor((self.cta_m_blocks // m_warps, 4), Uint32)
        if cutlass.const_expr(not self.route128_expand_b):
            tile_idx = Int32(0)
            while tile_idx < k_tiles:
                for pipe in cutlass.range_constexpr(self.pipeline_stages):
                    if tile_idx < k_tiles:
                        fetch_tile = tile_idx + Int32(1)
                        ...
                        compute_warp = Int32(1)
                        if cutlass.const_expr(self.route128_hybrid_tail):
                            if (
                                block_valid_rows <= Int32(64)
                                and warp_m != Int32(0)
                            ):
                                compute_warp = Int32(0)
                        for kk in cutlass.range_constexpr(self.cta_k_blocks):
                            if compute_warp != Int32(0):
                                for mb_local in cutlass.range_constexpr(
                                    self.cta_m_blocks // m_warps
                                ):
                                    mb = warp_m + Int32(mb_local * m_warps)
                                    if cutlass.const_expr(self.route128_hybrid_tail):
                                        if block_valid_rows <= Int32(64):
                                            mb = Int32(mb_local)
                                    a0, a1, a2, a3 = (
                                        self._load_a_registers_large_m(
                                            smem_base,
                                            a_sh_rd,
                                            Int32(pipe),
                                            Int32(kk),
                                    ...
                                    a_regs[mb_local, 0] = a0
                                    a_regs[mb_local, 1] = a1
                                    a_regs[mb_local, 2] = a2
                                    a_regs[mb_local, 3] = a3

                                q0, q1, q2, q3, s0, s1, s2, s3 = (
                                    self._load_b_registers_trellis256(
                                        smem_base,
                                        lane + warp_n * Int32(32),
                                        Int32(pipe),
                                        Int32(kk),
            ...
            self._store_tile_route128_warp_m(
                accs,
                c_bf16_flat,
                smem_base,
                tid,
        ...
        while tile_idx < k_tiles:
            for pipe in cutlass.range_constexpr(self.pipeline_stages):
                if tile_idx < k_tiles:
                    # Decode the complete compressed stage first.  All four
                    # physical warps contribute one N16 record from each N64
                    # group per K16 row.
                    for kk in cutlass.range_constexpr(self.cta_k_blocks):
                        for n_group in cutlass.range_constexpr(n_warps):
                            q0, q1, q2, q3, s0, s1, s2, s3 = (
                                self._load_b_registers_trellis256(
                                    smem_base,
                                    lane + Int32(n_group * 32),
                                    Int32(pipe),
                                    Int32(kk),
                                    reduce_k_tile + tile_idx,
                                    -1,
                            ...
                            matrix_lane_addr = (
                                smem_base
                                + Int32(self.sh_decode_b_off * 16)
                                + Int32(kk * self.tile_n * 32)
                                + Int32((n_group * 4) * 512)
                                + warp * Int32(512)
                                + (lane // Int32(8)) * Int32(128)
                                + (lane & Int32(7)) * Int32(16)
                            )
                            stmatrix_m8n8x4_b16(
                                matrix_lane_addr,
                                b_frag[0, 0],
                    ...
                    # Queue the next compressed-B/A stage before consuming
                    # the current expanded tile so cp.async overlaps the MMA
                    # mainloop.  route128_warp_m is gated to two stages.
                    fetch_tile = tile_idx + Int32(1)
                    if fetch_tile < k_tiles:
                        self._stage_k_tile_async(
                            a_bf16_flat,
                    ...
                    for kk in cutlass.range_constexpr(self.cta_k_blocks):
                        ...
                        for jj in cutlass.range_constexpr(4):
                            matrix_lane_addr = (
                                ...
                                + Int32(self.sh_decode_b_off * 16)
                                + Int32(kk * self.tile_n * 32)
                                + (warp_n * Int32(4) + Int32(jj)) * Int32(512)
                                + (lane // Int32(8)) * Int32(128)
                                + (lane & Int32(7)) * Int32(16)
                            )
    ...
    @cute.jit
    def _store_tile_route128_warp_m(
        self,
        accs,
        c_bf16_flat: cute.Tensor,
        smem_base: Int32,
        ...
        """Store the two M16 slices owned by each warp, then drain."""
        c_gl_stride = Int32(self.size_n // 8)
        c_sh_stride = Int32(2 * self.cta_n_blocks)
        rows_per_store_wave = self.cta_threads // (2 * self.cta_n_blocks)
        c_gl_wr_delta = c_gl_stride * Int32(rows_per_store_wave)
        c_sh_rd_delta = c_sh_stride * Int32(rows_per_store_wave)
        c_gl_wr = (
        ...
        c_sh_rd = (
            c_sh_stride * (tid // Int32(2 * self.cta_n_blocks))
            + (tid % Int32(2 * self.cta_n_blocks))
        )
        lane = tid & Int32(31)
        warp = tid >> Int32(5)
        ...
        warp_m = warp // Int32(n_warps)
        c_sh_wr_base = (
            Int32(4) * c_sh_stride * (lane // Int32(4))
            + lane % Int32(4)
            + Int32(32) * warp_n
        )
        ...
        for mb_local in cutlass.range_constexpr(self.cta_m_blocks // m_warps):
            mb = warp_m + Int32(mb_local * m_warps)
            compute_warp = Int32(1)
            if cutlass.const_expr(self.route128_hybrid_tail):
                if block_valid_rows <= Int32(64):
                    ...
                    if warp_m != Int32(0):
                        compute_warp = Int32(0)
            if compute_warp != Int32(0):
                c_sh_wr = c_sh_wr_base + mb * Int32(
                    16 * (4 * (2 * self.cta_n_blocks))
                )
    ...
    def _run_tile_large_m(
        ...
        accs = []
        for _ in cutlass.range_constexpr(self.cta_m_blocks):
            acc = [
                cute.make_rmem_tensor((_SCALAR_ACC_FRAGMENT_WIDTH,), cutlass.Float32)
                for _ in range(32 // _SCALAR_ACC_FRAGMENT_WIDTH)
            ]
        ...
        if cutlass.const_expr(
            ...
            if logical_k16 < Int32(8):
                for jj in cutlass.range_constexpr(4):
                    ...
                    self._mma_accumulate_m8(accs[0], jj, a_regs_cur, b_frag)
            else:
                for jj in cutlass.range_constexpr(4):
                    self._scaled_dequant_b_fragment_trellis256_bits(
                        b_frag,
        ...
        for jj in cutlass.range_constexpr(4):
            ...
            if cutlass.const_expr(uses_m_block_8):
                ...
                for mb in cutlass.range_constexpr(self.cta_m_blocks):
                    self._mma_accumulate_large_m(
                        accs[mb], a_regs_cur, mb, jj, b_frag
                    )
    ...
    def _finish_tile(
        ...
        if cutlass.const_expr(uses_m_block_8):
            self._fold_cta_partials_m8(accs[0], smem_base, tid)
        else:
            self._fold_cta_partials_large_m(accs, smem_base, tid)
        ...
        if cutlass.const_expr(uses_m_block_8):
            if tid < active_threads:
                for jj in cutlass.range_constexpr(4):
                    k = jj * 2
                    (
                        accs[0][(jj * 4) // _SCALAR_ACC_FRAGMENT_WIDTH][
                            (jj * 4) % _SCALAR_ACC_FRAGMENT_WIDTH
                        ],
                        accs[0][(jj * 4 + 1) // _SCALAR_ACC_FRAGMENT_WIDTH][
                            (jj * 4 + 1) % _SCALAR_ACC_FRAGMENT_WIDTH
                        ],
                        accs[0][(jj * 4 + 2) // _SCALAR_ACC_FRAGMENT_WIDTH][
                            (jj * 4 + 2) % _SCALAR_ACC_FRAGMENT_WIDTH
                        ],
                        accs[0][(jj * 4 + 3) // _SCALAR_ACC_FRAGMENT_WIDTH][
    ...
    def _prefetch_lookahead_tile(
        ...
        if fetch_tile < k_tiles:
            self._stage_k_tile_async(
                ...
                tid,
                Int32(
                    (pipe + self.pipeline_stages - 1) % self.pipeline_stages
                ),
                reduce_k_tile + fetch_tile,
                block_valid_rows,
                a_gl_stride,
...
class W4A16FusedMoeKernel:
    def __init__(
        ...
        *,
        ...
        fc1_packed_route_block_factor: int | None = None,
        fc1_pipeline_stages: int = _STAGES,
        fc2_moe_block_size: int | None = None,
        fc2_schedule_route_block_factor: int = 1,
        fc2_packed_route_block_factor: int | None = None,
        fc2_pipeline_stages: int = _STAGES,
        element_dtype: str = "bf16",
        fast_math: bool = True,
        swiglu_limit: float | None = None,
        swiglu_alpha: float | None = None,
        ...
        self.fc1_packed_route_block_factor = int(
            self.moe_block_size // self.fc1_moe_block_size
            if fc1_packed_route_block_factor is None
            else fc1_packed_route_block_factor
        )
        ...
        self.fc2_packed_route_block_factor = int(
            self.moe_block_size // self.fc2_moe_block_size
            if fc2_packed_route_block_factor is None
            else fc2_packed_route_block_factor
        )
        for name, compute_block, packed_factor in (
            (
                "FC1",
                self.fc1_moe_block_size,
                self.fc1_packed_route_block_factor,
            ),
            (
                "FC2",
                self.fc2_moe_block_size,
                self.fc2_packed_route_block_factor,
            ),
        ):
            if (
                compute_block not in _ALLOWED_ROUTED_SIZES
                or self.moe_block_size % compute_block != 0
                or packed_factor != self.moe_block_size // compute_block
            ):
                raise ValueError(
                    f"{name} route subtile must exactly cover the packed route "
                    f"block: packed={self.moe_block_size}, compute={compute_block}, "
                    f"factor={packed_factor}"
                )
        ...
        self.fc1 = W4A16GemmKernel(
            ...
            max_m_blocks=(
                max_m_blocks * self.fc1_packed_route_block_factor
            ),
            ...
            packed_route_block_factor=self.fc1_packed_route_block_factor,
            pipeline_stages=fc1_pipeline_stages,
        )
        self.fc2 = W4A16GemmKernel(
            ...
            packed_route_block_factor=self.fc2_packed_route_block_factor,
            pipeline_stages=fc2_pipeline_stages,
        )
...
def compile_w4a16_fused_moe(
    *,
    ...
    fc1_cols = int(intermediate_size) * (2 if is_gated else 1)
    routed_rows = int(size_m) * int(top_k)
    glm53_streamed_route128 = (
        bool(full_rotation)
        and int(moe_block_size) == 128
        and int(hidden_size) == 4096
        and int(intermediate_size) == 2048
        and int(num_experts) == 144
        and int(top_k) == 8
    ...
    route128_wide_experimental = (
        os.environ.get("B12X_GL53_ROUTE128_WIDE", "0") == "1"
    )
    route128_physical = bool(
        glm53_streamed_route128
        and force_tile_config is not None
        and (
            tuple(int(v) for v in force_tile_config)
            in ((128, 64, 128, 64), (64, 128, 64, 128))
            or (
                route128_wide_experimental
                and tuple(int(v) for v in force_tile_config)
                == (64, 256, 64, 256)
            )
        )
    )
    if route128_physical:
        fc1_compute_block_size = 128
        fc2_compute_block_size = 128
        fc1_pipeline_stages = 2
        fc2_pipeline_stages = 2
    else:
        fc1_compute_block_size = (
            64 if glm53_streamed_route128 else int(moe_block_size)
        )
        fc2_compute_block_size = (
            64 if glm53_streamed_route128 else int(moe_block_size)
        )
        fc1_pipeline_stages = 3 if glm53_streamed_route128 else _STAGES
        fc2_pipeline_stages = 3 if glm53_streamed_route128 else _STAGES
    ...
    fc1_tile_k, fc1_tile_n, fc1_cta_threads, _ = _select_tile_config(
        ...
        top_k=top_k,
        moe_block_size=fc1_compute_block_size,
        sms=sms,
        max_shared_mem=max_shared_mem,
        scale_format=scale_format,
        weight_layout=weight_layout,
    ...
    fc2_tile_k, fc2_tile_n, fc2_cta_threads, _ = _select_tile_config(
        ...
        top_k=1,
        moe_block_size=fc2_compute_block_size,
        sms=sms,
        max_shared_mem=max_shared_mem,
        scale_format=scale_format,
        weight_layout=weight_layout,
    ...
    if (
        ...
        if _candidate_tile_fits(
            problem_n=fc1_cols,
            problem_k=hidden_size,
            cta_m_blocks=_covering_count(fc1_compute_block_size, 16),
            tile_n=256,
    ...
    if (
        ...
        if _candidate_tile_fits(
            problem_n=hidden_size,
            problem_k=intermediate_size,
            cta_m_blocks=_covering_count(fc2_compute_block_size, 16),
            tile_n=256,
    ...
    if force_tile_config is not None:
        ...
        for name, forced_pn, forced_pk, forced_tn, forced_tk in (
            ...
            exact_route128_wide = bool(
                route128_wide_experimental
                and tuple(int(v) for v in force_tile_config)
                == (64, 256, 64, 256)
            )
            if not exact_route128_wide and not _candidate_tile_fits(
                problem_n=forced_pn,
                problem_k=forced_pk,
                cta_m_blocks=_covering_count(
                    fc1_compute_block_size
                    if name == "fc1"
                    else fc2_compute_block_size,
                    16,
                ),
                tile_n=forced_tn,
                tile_k=forced_tk,
                ...
                pipeline_stages=(
                    fc1_pipeline_stages if name == "fc1" else fc2_pipeline_stages
                ),
            ):
                raise ValueError(
    ...
    kernel = W4A16FusedMoeKernel(
        ...
        moe_block_size=moe_block_size,
        max_m_blocks=max_m_blocks,
        fc1_moe_block_size=fc1_compute_block_size,
        fc1_pipeline_stages=fc1_pipeline_stages,
        fc2_moe_block_size=fc2_compute_block_size,
        fc2_pipeline_stages=fc2_pipeline_stages,
        element_dtype=element_dtype,
        fast_math=fast_math,
        swiglu_limit=swiglu_limit,
        swiglu_alpha=swiglu_alpha,
...
@torch.library.custom_op(
    "trellismx_b12x::w4a16_small_m_direct_launch",
...
@torch.library.custom_op(
    "trellismx_b12x::w4a16_fc2_direct_launch",
    mutates_args="unknown",
)
...
@torch.library.custom_op(
    "trellismx_b12x::w4a16_fused_moe_launch",
    mutates_args="unknown",
)
...
@torch.library.custom_op(
    "trellismx_b12x::w4a16_fused_moe_calibrated_launch",
    mutates_args="unknown",
)
...
@torch.library.custom_op(
    "trellismx_b12x::w4a16_topk_sum_launch",
    mutates_args=("output",),
)
    ...
    *,
    ...
    if (not collect_activation_amax) and _small_m_direct_supported(
        ...
        if _small_m_direct_host_barrier_reset_enabled():
            barrier_count.zero_()
            barrier_epoch.zero_()
        torch.ops.trellismx_b12x.w4a16_small_m_direct_launch(
            a_input,
    ...
    if use_x4t_scale_predecode:
        ...
        from trellismx_b12x._lib.quant.x4t_scales import (
    ...
    if fused_launch is None:
        fused = compile_w4a16_fused_moe(
            ...
            coupled_hadamard=coupled_hadamard,
            rotation_input_dtype=rotation_input_dtype,
            force_tile_config=select_w4a16_full_rotation_launch_tile_config(
                storage_tile_config=prepared_tile_config,
                capacity_tokens=m,
                hidden_size=hidden_size,
                intermediate_size=intermediate_size,
                num_experts=int(prepared.num_experts),
    ...
    if collect_activation_amax:
        ...
        torch.ops.trellismx_b12x.w4a16_fused_moe_calibrated_launch(
            *launch_common,
            activation_amax,
            int(layer_idx_int),
            *launch_tail,
        ...
        torch.ops.trellismx_b12x.w4a16_fused_moe_launch(
            *launch_common,
            *launch_tail,
            bool(use_direct_topk_routes),
            bool(use_tc_decode),
    ...
    if full_rotation or sum_uses_map:
        ...
        torch.ops.trellismx_b12x.w4a16_topk_sum_launch(
            fc2_out,
            output,
            m,
            topk,
