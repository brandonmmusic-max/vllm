...
from trellismx_b12x._lib.utils import (
    current_cuda_stream,
...
from trellismx_b12x.moe._shared.routing import (
    route_topk as triton_route_topk,
)
from trellismx_b12x.moe._shared.kernels.relu2 import (
    MoEDynamicKernelRelu2,
    MoEMicroKernelRelu2,
)
from trellismx_b12x.moe._shared.kernels.silu import (
    MoEDynamicKernelSilu,
    MoEDynamicKernelSwiGLUOAI,
    MoEMicroKernelSilu,
    MoEMicroKernelSwiGLUOAI,
)
from trellismx_b12x.moe._shared.kernels.situ import (
...
from trellismx_b12x.moe._shared.execution import (
    MoEExecutionPlan,
    MoERegime,
    MoESpec,
...
from trellismx_b12x.moe._shared.trellis_codebooks import SQG_E4M3
from trellismx_b12x.moe._shared.tuning import lookup_max_active_clusters
from trellismx_b12x.moe._shared.dynamic_launch_tuning import (
    DynamicLaunchTuning,
...
from trellismx_b12x._lib.scratch import (
    ScratchBufferSpec,
    scratch_buffer_spec,
    scratch_tensor,
)
...
# The source-format vocabulary is owned by trellismx_b12x.moe._shared.execution;
...
def select_w4a16_full_rotation_tile_config(
    *,
    hidden_size: int,
    intermediate_size: int,
    num_experts: int,
    ...
    """Select a qualified full-rotation W4A16 weight/kernel tile.

    Keep one established N=256 storage layout. The exact GLM-5.3 route-128
    SMEM/register launch specialization is selected independently from this
    prepared-weight ABI.
    """
    ...
    if hidden_size % 128 or intermediate_size % 128:
        raise ValueError(
            "full-rotation W4A16 requires hidden and intermediate dimensions "
            "divisible by 128"
        )
    if device_major is None:
        resolved_device = torch.device("cuda" if device is None else device)
        if resolved_device.type != "cuda":
            raise ValueError("full-rotation W4A16 tile selection requires CUDA")
        device_major = int(torch.cuda.get_device_capability(resolved_device)[0])
...
class TPMoEScratchCaps:
    ...
    def __post_init__(self) -> None:
        ...
        if self.w4a16_block_size_m is not None:
            ...
            if block_size_m not in (8, 16, 32, 48, 64, 128):
                raise ValueError(
                    "w4a16_block_size_m must be one of 8, 16, 32, 48, 64, 128"
                )
...
def _get_activation_kernel_spec(
    ...
    *,
    ...
    if _normalize_quant_mode(quant_mode) == "w4a16":
        raise ValueError(
            "W4A16 dispatch uses trellismx_b12x.moe._shared.kernels.w4a16.kernel directly"
        )
    if (
...
def clear_tp_moe_caches() -> None:
    ...
    """
    from trellismx_b12x.moe._shared.kernels.w4a16.kernel import (
...
def _derive_w4a8_weight_grids(
    ...
    from trellismx_b12x.moe._shared.kernels.reference import (
...
def _prepare_w6a8_from_mxfp6_source(
    *,
    ...
    scales.  Returns a
    :class:`trellismx_b12x.moe._shared.kernels.w6a8.PreparedW6A8MXFP6Weights`.
    """
    from trellismx_b12x.moe._shared.kernels.w6a8 import prepare_w6a8_mxfp6_weights
...
def prepare_b12x_fp4_moe_weights(
    *,
    ...
    if plan.source_format == "btx":
        from trellismx_b12x.moe._shared.kernels.w4a16.btx import (
            BtxLayer,
        ...
        if manifest.rates.structure != (plan.trellis_rate_structure or "uniform"):
            raise ValueError(
    ...
    if WeightPreparationTransform.W4A16_NATIVE in plan.transforms:
        ...
        from trellismx_b12x.moe._shared.kernels.w4a16.prepare import (
...
def prewarm_w4a16_fc2_e8m0(experts, *, route_ids_dtype: torch.dtype) -> None:
    ...
    from trellismx_b12x.moe._shared.kernels.w4a16 import kernel as w4a16_kernel
    from trellismx_b12x.moe._shared.kernels.w4a16.prepare import W4A16FC2Weights
...
def run_w4a16_fc2_e8m0(
    ...
    *,
    ...
    from trellismx_b12x.moe._shared.kernels.w4a16 import kernel as _w4a16_kernel  # noqa: F401
    from trellismx_b12x.moe._shared.kernels.w4a16.prepare import W4A16FC2Weights
    ...
    torch.ops.trellismx_b12x.w4a16_fc2_direct_launch(
        intermediate,
        experts.w2,
        experts.w2_scale,
...
def _plan_full_rotation_w4a16_launches(
    *,
    ...
    from trellismx_b12x.moe._shared.kernels.w4a16.host import route_pack_capacity
    from trellismx_b12x.moe._shared.kernels.w4a16.kernel import (
        _DEFAULT_MAX_SHARED_MEM,
        compile_w4a16_fused_moe,
        compile_w4a16_topk_sum,
        pack_topk_routes_by_expert,
        select_w4a16_full_rotation_launch_tile_config,
    )
    ...
    with torch.cuda.device(core_plan.device):
        ...
        launch_tile_config = select_w4a16_full_rotation_launch_tile_config(
            storage_tile_config=core_plan.trellis_tile_config,
        ...
        def compile_fused(token_count: int) -> object:
            return compile_w4a16_fused_moe(
                ...
                force_tile_config=launch_tile_config,
...
def _resolve_trellis_route_block_size(caps: TPMoEScratchCaps) -> int | None:
    ...
    if _normalize_quant_mode(caps.quant_mode) != "w4a16":
        ...
        return None
    from trellismx_b12x.moe._shared.kernels.w4a16.host import select_route_block_size_m
...
@torch.library.custom_op(
    "trellismx_b12x::tp_moe_dynamic_launch",
...
def _launch_dynamic(
    *,
    ...
    torch.ops.trellismx_b12x.tp_moe_dynamic_launch(
...
def _get_tiny_decode_kernel(
    ...
    *,
    ...
    from trellismx_b12x.moe._shared.kernels.tiny_decode import (
...
@torch.library.custom_op(
    "trellismx_b12x::tp_moe_tiny_decode_launch",
    mutates_args="unknown",
)
...
@torch.library.custom_op(
    "trellismx_b12x::tp_moe_compact_micro_launch",
    mutates_args="unknown",
)
...
def _launch_micro(
    *,
    ...
    if quant_mode == "w4a8_mx" and not w4a8_trellis:
        ...
        torch.ops.trellismx_b12x.tp_moe_tiny_decode_launch(
            workspace.barrier_count,
            workspace.barrier_epoch,
    ...
    if flat_ids.dtype == torch.int32 and flat_ids.is_contiguous():
        ...
        launch_ids = workspace.compact_topk_ids[: flat_ids.numel()]
        launch_ids.copy_(flat_ids.to(torch.int32))
    torch.ops.trellismx_b12x.tp_moe_compact_micro_launch(
        workspace.barrier_count,
        workspace.barrier_epoch,
        workspace.micro_intermediate,
        weights.w1_storage,
