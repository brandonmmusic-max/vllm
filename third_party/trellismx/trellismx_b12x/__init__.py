"""trellismx_b12x — consumer-Blackwell (SM120/SM121) kernels.
...
collectives, ported from the trellismx_b12x project.  One grammar everywhere:

- ops live at ``trellismx_b12x.<group>.<op>`` and declare themselves via ``META``;
...
def find_op(qualname: str) -> OpMeta:
    ...
    if qualname not in _OPS:
        raise KeyError(
            f"unknown experimental trellismx_b12x op {qualname!r}; known ops: {sorted(_OPS)}"
        )
