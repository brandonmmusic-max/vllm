...
# part of every disk-cache key.  Must resolve to trellismx_b12x.
...
def _cute_compile_cache_dir() -> Path:
    ...
    if xdg_cache_home:
        return Path(xdg_cache_home) / "trellismx_b12x" / "compile"
    return Path.home() / ".cache" / "trellismx_b12x" / "compile"
...
def _format_cute_compile_stack() -> str:
    ...
    lines = ["[trellismx_b12x cute.compile] python_stack (most recent call last):"]
...
def _log_cute_compile_event(
    ...
    *,
    ...
    if _is_explicit_spec_payload(cache_payload):
        ...
        print(
            f"[trellismx_b12x cute.compile] {event} "
            f"{reason_text}"
...
def _call_cute_compile(
    ...
    *,
    ...
    print(
        f"[trellismx_b12x cute.compile] compile-start number={compile_number} {details}",
        flush=True,
    )
    ...
    try:
        ...
        print(
            f"[trellismx_b12x cute.compile] compile-failed number={compile_number} "
    ...
    with _COMPILE_PROGRESS_LOCK:
        ...
        total = _COMPILE_PROGRESS_TOTAL_SECONDS
    print(
        f"[trellismx_b12x cute.compile] compile-done number={compile_number} "
        f"duration_s={elapsed:.3f} total_compile_s={total:.3f} {details}",
        flush=True,
    )
    return compiled
...
def _function_fingerprint(func: Any) -> tuple[str, str, str]:
    ...
    if module.startswith("trellismx_b12x"):
        return module, qualname, f"trellismx_b12x:{_b12x_package_fingerprint()}"
    try:
...
def _build_compile_manifest(
    ...
    manifest: dict[str, Any] = {
        "schema": "trellismx_b12x._lib.compile_manifest.v3",
        "cache_key": cache_key,
        "cache_format": cache_format,
...
def _load_cute_compile_from_disk(cache_key: str):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(
            prefix="trellismx_b12x-cute-cache-load-"
        ) as raw_stage:
