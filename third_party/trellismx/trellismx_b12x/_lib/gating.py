"""Capability gating for trellismx_b12x.
...
def require_cutlass_dsl(minimum: str = MIN_CUTLASS_DSL) -> None:
    ...
    if version is None:
        raise ImportError(
            "trellismx_b12x requires nvidia-cutlass-dsl "
    ...
    if _version_tuple(version) < _version_tuple(minimum):
        raise ImportError(
            f"trellismx_b12x requires nvidia-cutlass-dsl >= {minimum}, "
...
def require_triton(op: str) -> None:
    if not has_triton():
        raise ImportError(
            f"trellismx_b12x.{op} requires triton (bundled with "
